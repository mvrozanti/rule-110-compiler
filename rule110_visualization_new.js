...
console.log('Rule 110 visualization script loaded - version 5');
...
function initWebGL() {
    ...
    if (!program) {
        ...
        program = null; // Make sure it's explicitly null
        return false;
    }
...
function requestDrawXXX() {
    console.log('requestDraw called from:', arguments.callee.caller);
    if (!drawRequested) {
        drawRequested = true;
...
function initApp() {
    ...
    function resizeCanvas() {
        ...
        if (canvas.width !== newWidth || canvas.height !== newHeight) {
            ...
            if (webglInitialized && gl && program && positionBuffer) {
                console.log('Updating WebGL uniforms for resize');
    ...
    // Event listeners - temporarily disabled for debugging
    /*
    ...
    container.addEventListener('mousemove', (e) => {
        if (isDragging && (webglInitialized || ctx)) {
...
if (document.readyState === 'loading') {
    ...
    console.log('Calling initApp immediately');
    initApp();
}
...
function drawMatrixXXX() {
    console.log('drawMatrix called with currentHistory:', currentHistory, 'caller:', arguments.callee.caller);
    console.trace('drawMatrix call stack');
    ...
    console.log('About to choose renderer: gl=', !!gl, 'program=', !!program, 'ctx=', !!ctx);
    if (gl && program) {
...
function drawMatrixCanvas2D() {
    console.log('drawMatrixCanvas2D called with currentHistory:', currentHistory);
    ...
    if (!currentHistory || !currentHistory.length) {
        console.log('No history data in Canvas 2D fallback');
        ctx.fillStyle = '#666';
        ctx.font = '16px monospace';
        ctx.textAlign = 'center';
        ctx.fillText('No data. Compile a program to see evolution.', canvas.width / 2, canvas.height / 2);
