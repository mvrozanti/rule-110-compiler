...
function drawBF(canvas, src, trace, t) {
  ...
  if (outY < h - 10) {
    ...
    const hasNL = raw.includes(10);
    ctx.fillStyle = raw.length ? "#9fe" : "#444";
    ctx.font = "12px ui-monospace, monospace";
    if (hasNL) {
      const text = String.fromCharCode(...raw);
      const lines = text.split("\n");
      const lineH = 14;
      const maxLines = Math.max(1, Math.floor((h - outY - 4) / lineH));
      const shown = lines.slice(-maxLines);
      for (let i = 0; i < shown.length; i++) {
        ctx.fillText(shown[i] || "", 60, outY + i * lineH);
      }
    } else {
      const display = bytesToDisplay(raw);
